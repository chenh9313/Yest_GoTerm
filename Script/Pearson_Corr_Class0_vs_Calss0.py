...
output_file = sys.argv[1] + "_pearson_corr_Class0_vs_Class0.txt"
...
with open(input_file, "r") as f:
    ...
    for line in f:
        ...
        gene = parts[0]
        gene_class = parts[1]   # Class column
        
        # Only keep genes with Class == 0
        if gene_class == "0":
            values = list(map(float, parts[2:]))  # numeric columns
            genes.append(gene)
            data.append(values)
